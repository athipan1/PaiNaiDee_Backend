...
class AnalyticsService:
        ...
        return [
            {
                ...
                "min_response_time": round(result.min_response_time, 2)
                if result.min_response_time
                else 0,
                "max_response_time": round(result.max_response_time, 2)
    ...
    def get_request_count_by_period(period="day", start_date=None, end_date=None):
        ...
        query = (
            ...
            .filter(
                APIAnalytics.timestamp >= start_date, APIAnalytics.timestamp <= end_date
            )
        ...
        return [
            {"period": str(result.period), "request_count": result.request_count}
            for result in results
        ]
    ...
    def get_status_code_distribution(start_date=None, end_date=None):
        ...
        query = db.session.query(
            APIAnalytics.status_code, func.count(APIAnalytics.id).label("count")
        )
        ...
        return [
            {
                ...
                "count": result.count,
                "percentage": round((result.count / total_count) * 100, 2)
                if total_count > 0
                else 0,
            }
            for result in results
    ...
    def get_system_overview(start_date=None, end_date=None):
        ...
        unique_ips = (
            query.filter(APIAnalytics.source_ip.isnot(None))
            .with_entities(distinct(APIAnalytics.source_ip))
            .count()
        )
        ...
        return {
            ...
            "latest_request": latest_request.timestamp.isoformat()
            if latest_request and latest_request.timestamp
            else None,
            "date_range": {
