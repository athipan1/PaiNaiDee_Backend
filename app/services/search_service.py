...
class SearchService:
    ...
    def calculate_similarity_score(
        self, query_terms: List[str], attraction: Attraction, use_pg_trgm: bool = False
    ...
    def search_attractions_with_fuzzy(
        ...
        if search_query.query and use_pg_trgm:
            ...
            expanded_terms = self.expand_query_with_synonyms(
                search_query.query, search_query.language
            )
            ...
            for term in expanded_terms[:3]:  # Limit to prevent overly complex queries
                similarity_conditions.extend(
                    [
                        ...
                        func.similarity(Attraction.description, term)
                        > similarity_threshold,
                        func.similarity(Attraction.province, term)
                        > similarity_threshold,
                    ]
                )
            ...
            results = [
                SearchResult(
                    attraction=attraction, similarity_score=1.0, matched_fields=["all"]
                )
                for attraction in attractions
            ]
        ...
        if search_query.min_rating is not None or search_query.max_rating is not None:
            ...
            for result in results:
                ...
                include = True
                if (
                    search_query.min_rating is not None
                    and rating < search_query.min_rating
                ):
                    include = False
                if (
                    search_query.max_rating is not None
                    and rating > search_query.max_rating
                ):
                    include = False
    ...
    def get_search_suggestions(
        self, query: str, language: str = "th", limit: int = 10
    ) -> List[Dict[str, Any]]:
