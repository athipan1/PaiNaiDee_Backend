...
class TalkService:
    ...
    def _generate_fallback_response(
        ...
        if any(word in message_lower for word in ["bangkok", "กรุงเทพ"]):
        ...
        if any(word in message_lower for word in ["beach", "ชายหาด", "sea"]):
        ...
        if any(word in message_lower for word in ["food", "อาหาร", "eat"]):
    ...
    def generate_response(
        self, sender: str, receiver: str, message: str, session_id: Optional[str] = None
    ) -> Dict:
        ...
        try:
            ...
            if not self.client:
                ...
                if session_id:
                    self._update_session_context(
                        session_id, message, reply, sender, receiver
                    )
            ...
            response = self.client.chat.completions.create(
                ...
                stop=[
                    f"[{sender}]:"
                ],  # Stop if the model tries to respond as the sender
            )
