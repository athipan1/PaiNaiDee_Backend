...
class Config:
    ...
    def SEARCH_RANK_WEIGHTS(self):
        ...
        weights_str = os.environ.get(
            "SEARCH_RANK_WEIGHTS",
            '{"popularity": 0.4, "freshness": 0.3, "similarity": 0.3}',
        )
        try:
    ...
    FEATURE_AUTOCOMPLETE = (
        os.environ.get("FEATURE_AUTOCOMPLETE", "true").lower() == "true"
    )
    ...
    def validate_required_config(cls):
        ...
        if not (0.9 <= total_weight <= 1.1):  # Allow small floating point errors
            raise ValueError(
                f"Search rank weights must sum to approximately 1.0, got {total_weight}"
            )
...
class ProductionConfig(Config):
    ...
    RATE_LIMIT_PER_MINUTE = int(
        os.environ.get("RATE_LIMIT_PER_MINUTE", 30)
