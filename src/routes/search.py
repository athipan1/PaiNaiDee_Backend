...
def get_search_suggestions():
    ...
    if query:
        ...
        for place in places:
            name_to_show = (
                place.name_local
                if language == "th" and place.name_local
                else place.name
            )
            suggestions.append(
                {
                    ...
                    "text": name_to_show,
                    "description": (
                        place.province_local if language == "th" else place.province
                    ),
                    "province": (
                        place.province_local if language == "th" else place.province
                    ),
        ...
        for province in provinces:
            name_to_show = (
                province.name_local
                if language == "th" and province.name_local
                else province.name
            )
            suggestions.append(
                {
        ...
        for category in categories:
            name_to_show = (
                category.name_local
                if language == "th" and category.name_local
                else category.name
            )
            suggestions.append(
                {
        ...
        tags = (
            ...
            .filter(
                (Tag.name.ilike(f"%{query}%")) | (Tag.name_local.ilike(f"%{query}%"))
            )
            .limit(5)
            .all()
        )
        for tag in tags:
            name_to_show = (
                tag.name_local if language == "th" and tag.name_local else tag.name
            )
            suggestions.append(
                {
                    "id": f"tag-{tag.id}",
